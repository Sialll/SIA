...
- 단계 2 `세션 분리`: 완료
- 단계 3 `슬리피지 / 수수료`: 1차 완료
  - 기본 비용 가정이 가격/포지션 백테스트에 반영됨
  - 현재는 `순수익(net)` 중심 표시
  - 다음 보완은 `gross / net 동시 표기`
...
- `종목별 평균 edge`
- `상위 종목 집중도 비율`
  - 예: 상위 1종목 / 3종목 / 5종목이 전체 signal edge에서 차지하는 비중
- `최소 표본 조건`
  - trade 수가 너무 적은 종목은 별도 표시
- `watchlist subset 비교`
  - `M7`
  - `비금융 대형주`
  - `금융`
  - `KR 대형주`
  같은 운영 집합별 성과 비교 훅 확보
...
다음 실제 구현은 `단계 4. 종목 편향 진단`이 맞다.

이유:

- 단계 1~3이 이미 들어가서 숫자의 의미는 크게 좋아졌다.
- 이제 판매 설명에서 제일 약한 부분은 `몇 종목이 성과를 끌고 가는가`다.
- Phase 4가 들어가면 `소수 종목 착시`에 대한 방어가 가능해진다.
