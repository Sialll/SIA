...
dump_preflight_failure() {
  local attempt="$1"
  local out_file="$2"
  local parse_log="$3"
  local check_log="$4"
  echo "==== gate failure context (attempt ${attempt}) ===="
  echo "preflight log: ${out_file}"
  echo "parse log: ${parse_log}"
  echo "check log: ${check_log}"
  echo "---"
  log_snippet "preflight log (last 160 lines)" "$out_file" 160
  echo "---"
  log_snippet "parse-issue log (last 120 lines)" "$parse_log" 120
  echo "---"
  log_snippet "check-pr log (last 120 lines)" "$check_log" 120
}
...
while true; do
  ...
  if [[ "$AUTO_TEMPLATE" != true || "$attempt" -gt 1 || -z "$PR_NUMBER" ]]; then
    if [[ "$PRE_FAILED_DUMPED" -eq 0 ]]; then
      dump_preflight_failure "$attempt" "$PREV_OUT" "$PREV_PARSE_LOG" "$PREV_CHECK_LOG"
      PRE_FAILED_DUMPED=1
    fi
    break
  fi
  ...
  if [[ "$attempt" -gt "$MAX_ATTEMPTS" ]]; then
    if [[ "$PRE_FAILED_DUMPED" -eq 0 ]]; then
      dump_preflight_failure "$((attempt - 1))" "$PREV_OUT" "$PREV_PARSE_LOG" "$PREV_CHECK_LOG"
      PRE_FAILED_DUMPED=1
    fi
    break
  fi
...
if [[ "$PREV_RC" -ne 0 && "$PRE_FAILED_DUMPED" -eq 0 ]]; then
  dump_preflight_failure "$attempt" "$PREV_OUT" "$PREV_PARSE_LOG" "$PREV_CHECK_LOG"
fi
