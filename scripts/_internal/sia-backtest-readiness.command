...
REPORT_FILE="${CACHE_DIR}/backtest-readiness-report.html"
...
"$PYTHON_BIN" "${PROJECT_ROOT}/src/sia/backtest_readiness_report.py" \
  --db-path "$DB_PATH" \
  --output "$REPORT_FILE"

if [[ "${SIA_NO_OPEN_BACKTEST_READINESS:-0}" != "1" ]] && command -v open >/dev/null 2>&1; then
  open "$REPORT_FILE" >/dev/null 2>&1 || true
fi

echo "백테스트 준비도 리포트: $REPORT_FILE"
