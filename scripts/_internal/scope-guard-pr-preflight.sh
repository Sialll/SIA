...
if [[ -n "$BODY_OVERRIDE" ]]; then
  ...
  PR_HAS_TOP_LEVEL_HEADING=false
  if printf '%s\n' "$PR_BODY" | awk 'BEGIN {found=0} {if (found) next} NF>0 {if ($0 ~ /^[[:space:]]*# /) {found=1}; exit} END {exit (found ? 0 : 1)}'; then
    PR_HAS_TOP_LEVEL_HEADING=true
  fi

  if [[ -n "$PR_TITLE" ]] && [[ "$PR_HAS_TOP_LEVEL_HEADING" == false ]]; then
...
if [[ "$PARSE_RC" -ne 0 ]]; then
  ...
  print_log_snippet "parse-issue log (last 120 lines)" "$PARSE_LOG" 120
else
...
if [[ "$RUN_CHECK_PR" == true && -n "$PR_NUMBER" ]]; then
  ...
  if [[ -n "$FILE_LIST" ]]; then
    ...
    if "$PY_BIN" -m src.sia.cli check-pr --files "$FILE_LIST" > "$CHECK_LOG" 2>&1; then
      if check_pr_ok "$CHECK_LOG"; then
        echo "check-pr: OK"
        cat "$CHECK_LOG"
      else
        CHECK_RC=$?
        echo "check-pr: FAILED (exit=$CHECK_RC)"
        print_log_snippet "check-pr log (last 120 lines)" "$CHECK_LOG" 120
      fi
    else
      CHECK_RC=$?
      echo "check-pr: FAILED (exit=$CHECK_RC)"
